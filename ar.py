...
with tabs[0]:
    ...
    if ranking_type == "Doubles":
        ...
        if matches.empty or players_df.empty:
            ...
            st.markdown("---")
            with st.expander("Process being used for Rankings",expanded=False):
                st.markdown("""
                ...
                Technical Over view with code references
The technical summary of the point allocation and ranking calculation logic used in the AR
Tennis application.
1. Core Ranking Logic : calculate_rankings()
The entire ranking process is handled by the calculate_rankings(matches_to_rank)
function. This function takes a pandas DataFrame of matches as input and outputs a ranked
DataFrame of players along with detailed partnership statistics.
Data Accumulation
The function initializes several defaultdict objects from Python's collections module to
store player statistics as it iterates through matches. This is efficient as it automatically handles
the creation of a new entry for any player encountered for the first time.
• scores = defaultdict(float): Stores the primary ranking points for each player.
• wins = defaultdict(int): Counts the total number of matches won.
• losses = defaultdict(int): Counts the total number of matches lost.
• matches_played = defaultdict(int): Counts the total number of matches a
player has participated in.
• games_won = defaultdict(int): Accumulates the total number of games a player
has won across all sets.
• game_diff = defaultdict(float): Accumulates a player's average game
difference from each match.
• partner_stats: A nested defaultdict to store detailed statistics for doubles
partnerships.
Per-Match Processing
The function iterates through each match in the matches_to_rank DataFrame. In each
iteration, it performs the following steps:
1. Game Difference Calculation:
• For each match, it calculates the total game difference from the perspective of
Team 1 (match_gd_sum). It does this by parsing the score of each valid set (e.g.,
'6-4' results in a difference of +2).
• It then computes the average game difference for that specific match
(match_gd_avg) by dividing match_gd_sum by the number of sets played. This
normalizes the impact of two-set versus three-set matches.2. Point & Statistic Allocation:
• The system allocates points based on the match outcome (winner column):
• Win: Players on the winning team each receive 3.0 points. Their wins count
is incremented.
• Loss: Players on the losing team each receive 1.0 point. Their losses count
is incremented.
• Tie: All players in the match receive 1.5 points.
• For every match, the matches_played count is incremented for all participants.
• The game_diff for each player is updated by adding or subtracting the
match_gd_avg calculated for that match. Winners have it added, and losers have
it subtracted.
• The games_won for each player is updated by summing the raw number of games
they won in each set.
2. Final Ranking and Tie-Breaking
After processing all matches, the function aggregates the collected data to produce the final
rankings.
1. Final Stat Calculation: It iterates through every player recorded in the scores dictionary
and calculates final metrics:
• Win %: Calculated as (wins / matches_played) * 100.
• Game Diff Avg: The total accumulated game_diff is divided by
matches_played. This results in the player's average game differential across all
their matches.
2. Sorting and Ranking: A new DataFrame (rank_df) is created. The final rank is determined
by sorting this DataFrame based on a strict hierarchy of criteria. This tie-breaking logic is
critical:
• Primary Key: Points (descending)
• Secondary Key: Win % (descending)
• Tertiary Key: Game Diff Avg (descending)
• Quaternary Key: Games Won (descending)
• Final Tie-Breaker: Player name (ascending, for alphabetical consistency)
Once sorted, a "Rank" column is generated based on the player's position in the sorted
DataFrame.3. Ancillary Functions
get_player_trend()
The "Recent Trend" string (e.g., "W L W W L") displayed in the rankings is generated by the
get_player_trend(player, matches, max_matches=5) function. This function
isolates a player's most recent five matches, determines the outcome ('W' or 'L') for each, and
concatenates them into a display string. This is purely for informational purposes and does not
affect the point calculation or ranking order.
4. Application Context and Filtering
In the main application, the calculate_rankings() function is called within the "Rankings"
tab. The user's selection from the ranking_type radio buttons ("Combined", "Doubles",
"Singles") determines which matches are passed to the function.
• If "Doubles" is selected, the main matches DataFrame is first filtered to include only rows
where match_type == 'Doubles'.
• If "Singles" is selected, it's filtered for match_type == 'Singles'.
• If "Combined" is selected, the entire matches DataFrame is passed without filtering.
This mechanism allows the single, robust calculate_rankings() function to generate
different, context-specific leaderboards based on a filtered subset of the overall match data.
