...
with tabs[0]:
    st.header(f"Rankings as of {datetime.now().strftime('%d/%m')}")
                        ...
                        if not court_edit:
                            ...
                            if match_type_edit == "Doubles" and len(set(selected_players_edit)) != len(selected_players_edit):
                                ...
                                try:
                                    ...
                                    save_bookings(bookings_to_save)
                                    load_bookings()
                                    st.write(f"Refreshed booking row: {st.session_state.bookings_df[st.session_state.bookings_df['booking_id'] == booking_id].to_dict('records')}")
