...
with tabs[0]:
    ...
    rank_df_doubles, _ = calculate_rankings(st.session_state.matches_df[st.session_state.matches_df["match_type"] == "Doubles"])
    rank_df_singles, _ = calculate_rankings(st.session_state.matches_df[st.session_state.matches_df["match_type"] == "Singles"])
    
    # Generate PDF button for rankings
    if not (rank_df_combined.empty and rank_df_doubles.empty and rank_df_singles.empty):
        ...
        st.download_button(
            ...
            help="Download the current rankings as a PDF file."
        )
    ...
    # Tabs for Combined, Doubles, Singles rankings
    rank_tabs = st.tabs(["Combined", "Doubles", "Singles"])
    ...
    with rank_tabs[0]:  # Combined Rankings
        filtered_matches = st.session_state.matches_df
        if filtered_matches.empty:
            st.info("No match data available for combined rankings.")
        else:
            # --- Top 3 Players Display (only for Combined) ---
            if not rank_df_combined.empty:
                ...
                for _, player_data in top_3_players.iterrows():
                    ...
                    card = (
                        f'<div class="top-player-card">'
                        f'<img src="{profile_image_url}" alt="Profile">'
                        f'<h3>🏆 {rank}</h3>'
                        f'<p><b>{player_name}</b></p>'
                        f'</div>'
                    )
            ...
            st.subheader("Combined Rankings Table")
            st.markdown('<div class="rankings-table-container">', unsafe_allow_html=True)
            st.markdown('<div class="rankings-table-scroll">', unsafe_allow_html=True)
            
            for index, row in rank_df_combined.iterrows():
                profile_html = (
                    f'<a href="{row["Profile"]}" target="_blank">'
                    f'<img src="{row["Profile"]}" class="profile-image" alt="Profile"></a>'
                    if row["Profile"]
                    else ''
                )
                player_styled = f"<span style='font-weight:bold; color:#fff500;'>{row['Player']}</span>"
                ...
                st.markdown(
                    ...
                    f'<div class="rank-profile-player-group">'
                    f'<div class="rank-col">{row["Rank"]}</div>'
                    f'<div class="profile-col">{profile_html}</div>'
                    f'<div class="player-col">{player_styled}</div>'
                    f'</div>'
                    f'<div class="points-col">{points_value_styled}</div>'
                    f'<div class="win-percent-col">{row["Win %"]:.1f}%</div>'
                    f'<div class="matches-col">{int(row["Matches"])}</div>'
                    f'<div class="wins-col">{int(row["Wins"])}</div>'
                    f'<div class="losses-col">{int(row["Losses"])}</div>'
                    f'<div class="game-diff-avg-col">{row["Game Diff Avg"]:.2f}</div>'
                    f'<div class="games-won-col">{int(row["Games Won"])}</div>'
                    f'<div class="trend-col">{trend_value_styled}</div>'
                    f'</div>',
                    unsafe_allow_html=True
                )
            st.markdown('</div>', unsafe_allow_html=True)
            ...
            if selected_player_rankings:
                display_player_insights(
                    ...
                    filtered_matches,
                    rank_df_combined,
                    partner_stats_combined,
                    key_prefix="rankings_combined_"
                )
            else:
                st.info("Player insights will be available once a player is selected.")
            
            # Display the nerd stats chart
            fig = create_nerd_stats_chart(rank_df_combined)
            if fig:
                st.plotly_chart(fig, use_container_width=True)
            
            # Display partnership performance chart
            if selected_player_rankings:
                fig_partner = create_partnership_chart(selected_player_rankings, partner_stats_combined, st.session_state.players_df)
                if fig_partner:
                    st.plotly_chart(fig_partner, use_container_width=True)

    with rank_tabs[1]:  # Doubles Rankings
        filtered_matches = st.session_state.matches_df[st.session_state.matches_df["match_type"] == "Doubles"]
        if filtered_matches.empty:
            st.info("No doubles match data available.")
        else:
            st.subheader("Doubles Rankings Table")
            st.markdown('<div class="rankings-table-container">', unsafe_allow_html=True)
            st.markdown('<div class="rankings-table-scroll">', unsafe_allow_html=True)
            
            for index, row in rank_df_doubles.iterrows():
                profile_html = (
                    f'<a href="{row["Profile"]}" target="_blank">'
                    f'<img src="{row["Profile"]}" class="profile-image" alt="Profile"></a>'
                    if row["Profile"]
            ...
            selected_player_rankings = st.selectbox(
                ...
                [""] + players,
                index=0,
                key="insights_player_rankings_doubles"
            )
            if selected_player_rankings:
                display_player_insights(
                    selected_player_rankings,
                    ...
                    rank_df_doubles,
                    partner_stats_combined,
                    key_prefix="rankings_doubles_"
                )
            else:
                st.info("Player insights will be available once a player is selected.")
            
            # Display the nerd stats chart
            fig = create_nerd_stats_chart(rank_df_doubles)
            if fig:
                st.plotly_chart(fig, use_container_width=True)
    ...
    with rank_tabs[2]:  # Singles Rankings
        filtered_matches = st.session_state.matches_df[st.session_state.matches_df["match_type"] == "Singles"]
        if filtered_matches.empty:
            st.info("No singles match data available.")
        else:
            st.subheader("Singles Rankings Table")
            st.markdown('<div class="rankings-table-container">', unsafe_allow_html=True)
            st.markdown('<div class="rankings-table-scroll">', unsafe_allow_html=True)
            
            for index, row in rank_df_singles.iterrows():
                profile_html = (
                    f'<a href="{row["Profile"]}" target="_blank">'
                    f'<img src="{row["Profile"]}" class="profile-image" alt="Profile"></a>'
                    if row["Profile"]
            ...
            selected_player_rankings = st.selectbox(
                ...
                [""] + players,
                index=0,
                key="insights_player_rankings_singles"
            )
            if selected_player_rankings:
                display_player_insights(
                    selected_player_rankings,
                    ...
                    rank_df_singles,
                    partner_stats_combined,
                    key_prefix="rankings_singles_"
                )
            else:
                st.info("Player insights will be available once a player is selected.")
            
            # Display the nerd stats chart
            fig = create_nerd_stats_chart(rank_df_singles)
            if fig:
                st.plotly_chart(fig, use_container_width=True)
